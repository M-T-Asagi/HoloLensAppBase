...
public class CustomMessages : Singleton<CustomMessages>
{
    ...
    private Dictionary<CustomMessageID, MessageCallback> _MessageHandlers = new Dictionary<CustomMessageID, MessageCallback>();
    public Dictionary<CustomMessageID, MessageCallback> MessageHandlers
    {
        get
        {
    ...
    void InitializeMessageHandlers()
    {
        ...
        for (byte index = (byte)CustomMessageID.HeadTransform; index < (byte)CustomMessageID.Max; index++)
        {
            if (MessageHandlers.ContainsKey((CustomMessageID)index) == false)
            {
    ...
    public void SendHeadTransform(Vector3 position, Quaternion rotation, byte HasAnchor)
    {
        ...
        if (this.serverConnection != null && this.serverConnection.IsConnected())
        {
            ...
            NetworkOutMessage msg = CreateMessage((byte)CustomMessageID.HeadTransform);
